...
class Mouse protected () extends qx.core.Object with qx.event.IEventHandler {
    ...
    protected def _onButtonEvent(domEvent: qx.event.typ.Event): Unit = js.native
    protected def _onMoveEvent(domEvent: qx.event.typ.Event): Unit = js.native
    protected def _onWheelEvent(domEvent: qx.event.typ.Event): Unit = js.native
